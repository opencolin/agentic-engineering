...
How agents remember things across turns and across sessions. Distinct from [Context Engineering](/context-engineering/) — context engineering is about what's in the window *right now*; memory is about what survives when the window resets.
...
- [Context Engineering](/context-engineering/) — what's loaded into the window right now (memory's read side)
- [Harness Engineering](/harness-engineering/) — the system that orchestrates memory reads/writes
- [Skills](/skills/) — skills can encode procedural memory; this page covers everything else
- [Infrastructure: Hosting & Execution](/infrastructure/) — where memory persistence layers actually run
- [Research Notes](/research-notes/) — primary sources for the vendors and patterns on this page
