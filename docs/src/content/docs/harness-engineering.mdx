...
The canonical public corpus of skill examples is **[anthropics/skills](https://github.com/anthropics/skills)** (140K stars) — the `SKILL.md` format spec, ~50 reference skills (docx / pdf / pptx / xlsx, MCP server generation, brand-asset workflows), and the template the [Skills, Plugins & Marketplaces](/approaches/#skills-plugins-marketplaces) ecosystem builds against. The DOCX / PDF / PPTX skills are worth reading end-to-end as examples of *progressive disclosure done well* — each skill is 50–200 lines, declares its triggers in YAML frontmatter, and defers deep references to topic files the agent loads on demand.
...
| First-party agent SDKs (loop + tools + hooks pre-built) | [Claude Agent SDK](/approaches/#claude-agent-sdk) (Anthropic, MIT Python + 7K stars / 1.5K TS — the same harness inside Claude Code), [OpenAI Agents SDK](/approaches/#openai-agents-sdk) (27K), [Google ADK](/approaches/#google-adk-agent-development-kit) (20K), [Strands Agents](/approaches/#strands-agents) (AWS, 5.9K) | [Approaches: Frameworks & SDKs](/approaches/) |
| Open packaged harness on LangGraph | [Deep Agents (LangChain)](/approaches/#deep-agents-langchain) — model-neutral, batteries-included; planning + virtual FS + sub-agents + summarization as composable middleware; v0.5 alpha April 2026 | [Approaches: Deep Agents](/approaches/#deep-agents-langchain) |
| End-to-end reference harness (skill pack) | [GStack](/approaches/#gstack) — Garry Tan's 23-skill Claude Code setup, MIT, 102K stars; one-command install, paired with Conductor for parallel worktrees | [Approaches: GStack](/approaches/#gstack) |
| End-to-end reference harness (memory + ops side) | [GBrain](/approaches/#gbrain) — Garry Tan's persistent-memory companion: typed knowledge graph + 29 skills + Postgres-native "Minions" job queue; MIT, 19K stars; *"GStack is the engine, GBrain is the mod"* | [Approaches: GBrain](/approaches/#gbrain) |
