...
- **Progressive skill disclosure.** [Skills](/skills/) load metadata at startup, full SKILL.md when relevant, bundled files only when actually needed.
...
- [Harness Engineering](/harness-engineering/) — the surrounding system that implements these strategies
- [Skills](/skills/) — the cross-vendor primitive for the "select" strategy at the capability layer
- [Tool Design](/tool-design/) — tool-layer compression and the "select" lever for actions
- [Memory](/memory/) — the durable substrate for the "write" strategy
- [Research Notes](/research-notes/) — primary-source bibliography for every claim on this page
