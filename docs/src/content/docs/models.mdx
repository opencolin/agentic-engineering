...
5. **Audit `agent × model`, not `model`.** Same Opus 4.5 scores 78% inside Claude Code and 42% inside Smolagents on [CORE](/benchmarks/#other-benchmarks-worth-knowing) — see [Harness Engineering](/harness-engineering/). The harness is often a bigger lever than the model upgrade you were about to buy.
...
| **Claude Sonnet 4.6** | Anthropic | Coding workhorse | Default model inside Claude Code, Cursor, Amp; strong on SWE-bench Verified | [pricing](https://platform.claude.com/docs/en/about-claude/pricing) |
...
- **Hosted-only "Operator-class" agents.** Claude Cowork, OpenAI Operator, Anthropic Computer Use — these are *products*, not exposed as raw model APIs; covered in [Approaches](/approaches/#browser-use--computer-use-frameworks).
...
- [Inference](/inference/) — providers, routers, and how to actually serve any of these models
- [Benchmarks](/benchmarks/) — what the scores in this table mean
