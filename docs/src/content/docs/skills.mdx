...
This is why complexity isn't bounded by context size. The PDF-skill example: it can manipulate PDFs (fill forms, extract fields) via a bundled Python script that the model invokes — *the PDF itself never enters the context window*. Deterministic operations run outside generation in [code execution](/tool-design/).
...
[Anthropic's Auto Mode](https://anthropic.com/engineering/claude-code-auto-mode) layers a transcript classifier on top to catch overeager skill invocations — see [Harness Engineering](/harness-engineering/#sandboxing--permissions) for the broader security model.
...
- [Context Engineering](/context-engineering/) — skills are the progressive-disclosure implementation of "select"
- [Tool Design](/tool-design/) — skills bundle tools; this page covers what makes a *single* tool good
- [Harness Engineering](/harness-engineering/) — where skills slot into the broader system
- [Research Notes](/research-notes/) — primary sources for every claim on this page
