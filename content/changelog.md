...
New [Sandboxes § Environment Classes](sandboxes.md#environment-classes-beyond-code-sandboxes) section from [TokenJam's environments explainer](https://tokenjam.dev/blog/2026-05-18-what-are-agent-environments-and-sandboxes) (Anil Murty, May 2026). The page previously covered only the code-sandbox class; the new section adds the other three: **browser sandboxes** (Browserbase + Stagehand), **virtual desktops** (Computer Use / Operator, with the "right tool when no API exists, wrong tool when one does" rule), and **simulation environments** (HUD, Inspect AI Sandboxing Toolkit). Two load-bearing ideas integrated: *environments and evaluation share infrastructure* — same sandbox, deterministic data, full instrumentation, so "can I replay a production run as an eval" becomes a vendor-selection criterion — and the *failures of grounding, not intent* framing for why isolation matters, now in the Why Sandboxes Matter opening. Firecracker per-step-isolation numbers (~125ms boot, 3–10MB) and the "AWS Lambda's security model applied to agents" line added with the isolation-is-a-spectrum caveat.

### Stripe Minions section enriched from three sources
[Approaches § Stripe Minions](approaches.md#stripe-minions) rebuilt from the primary sources: Stripe's [Part 1 blog post](https://stripe.dev/blog/minions-stripes-one-shot-end-to-end-coding-agents) (Alistair Gray, Leverage team, Feb 2026), the [Part 2 implementation post](https://stripe.dev/blog/minions-stripes-one-shot-end-to-end-coding-agents-part-2), [MindStudio's blueprint-architecture analysis](https://www.mindstudio.ai/blog/stripe-minions-blueprint-architecture-deterministic-agentic-nodes), and the [How I AI interview](https://www.youtube.com/watch?v=o5Mi5SYSDnY). New: the why-in-house rationale ($1T+/yr payment volume, Ruby-sans-Rails + Sorbet, homegrown libraries), the three-rung feedback ladder (sub-5-second local lint → selective CI from 3M+ tests with auto-applied autofixes → max-2-rounds rationale), the product surface (Slack threads, flaky-test tickets with a fix-with-a-minion button, follow-up instructions on completed runs), and MindStudio's deterministic-vs-agentic node taxonomy with bounded retry loops. **Correction:** Toolshed hosts 400+ MCP tools per the primary source, not ~500 as previously stated (fixed in four places). Who's Who updated — the Minions post is credited to Alistair Gray, not anonymous.

### Cerebral Valley sweep: 23 events added, OpenAI DevDay date corrected
Second events pass of the day, sourced from the [Cerebral Valley events list](https://cerebralvalley.ai/events). **Correction: OpenAI DevDay 2026 is Sep 29 (SF), not Nov 4** as previously listed. Added 13 July agent hackathons and meetups (Computer Use Agents Hackathon by H Company, Enterprise Agents Hackathon by Emergence × Nebius, Hermes Buildathon, the Fermi long-horizon-reliability qualifier, Loop Engineering Hackathon, Agent Swarms, You.com Agentic Hackathon, Daytona × Braintrust HackSprint, Senior SWE-Bench reading group, and more) plus 10 fall conferences: **Agentic AI Summit at UC Berkeley** (Aug 1–2, flagship), ModCon, HumanX Amsterdam, WeAreDevelopers NA, The AI Conference SF, AI Native Summit, **AGNTCon + MCPCon** (Oct 22 — the verifiable successor to the removed MCP Summit entry), QCon SF, Agentic AI Summit Toronto, and The AI Summit New York. Off-topic listings (networking mixers, dinners, non-AI verticals) skipped. 43 upcoming events now listed.

### AI/HPC infrastructure conferences added to Events
Seven AI-infrastructure conferences added to the [Events](events.md) calendar, sourced from a conference-sponsorship tracker and verified against official sites before adding: **AMD Advancing AI** (SF, Jul 22–23), **AI4** (Las Vegas, Aug 4), **AI Infra Summit** (Santa Clara, Sep 15–17), **Xcelerated Compute Show** (London, Sep 16–17), **Fully Connected 26 / CoreWeave** (SF, Sep 29), **OCP Global Summit** (San Jose, Oct 15), and **SC26** (Chicago, Nov 15–20). These extend the calendar's coverage into the AI-infra layer the site covers in [Inference](inference.md) and [Infrastructure](infrastructure.md). Skipped from the same source: Bitcoin/mining/energy conferences (off-topic) and two events whose domains are dead or for sale (Yotta, AGI Summit). 20 upcoming events now listed.
...
New top-level section in [Infrastructure § CI Runners for Agent Iteration](infrastructure.md#ci-runners-for-agent-iteration) placed between Cloud Mac Hosting and Self-Hosted Infrastructure. Opens with the framing that **CI is the agent's feedback loop** — when an agent fleet opens 30 PRs an hour and each waits 8 minutes on a GitHub-hosted runner, the harness is throttled by the runner queue, not the model. Covers seven drop-in `runs-on:` replacements: **Blacksmith** (YC W24, $10M Series A Sep 2025 GV-led, "Unblock AI Development with Fast CI" — used by Vercel / Supabase / Clerk / Mercury / Ashby), **Tenki Runners** (Luxor / Seattle, single-tenant ephemeral VMs, same operator as the Tenki Sandbox for agent code execution), **Depot** (Docker / Bazel cache-first), **Namespace** (runners + remote dev envs), **BuildJet** (the mature conservative pick), **RunsOn** (self-hosted on your AWS), and **Ubicloud** (OSS, "open AWS"). Includes a choosing rubric and cross-references to [Mac CI Runners](infrastructure.md#mac-ci-runners) and the [Sandboxes branching-as-replay-buffer pattern](sandboxes.md#the-sandbox-market-structure). Fills a previously missing leg of the agent-iteration stack — the site already covered sandboxes, dev environments, and orchestration but not CI itself as an agent infrastructure category.
