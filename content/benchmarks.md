...
Notable pattern: the same agent harness (ForgeCode) ranks #1 and #3 on different models — reinforcing the [harness-is-a-bigger-lever-than-model](patterns.md) thesis. Smaller models (GPT-5-Nano, OSS-20B) score in single digits regardless of harness, so scaffolding alone doesn't rescue a weak base.
...
SWE-bench / Terminal Bench score on pass-rate against a deterministic harness. τ-bench scores on **Pass^k** — the probability the agent succeeds on *k* independent attempts. That metric matters because customer-service agents fail differently each run; a 70% pass@1 with 30% Pass^4 is a different product than 70% pass@1 with 65% Pass^4. Sierra publishes the leaderboard and uses it as the basis for their own product claims.
