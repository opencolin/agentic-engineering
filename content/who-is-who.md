...
- **Stripe Leverage team** (Alistair Gray et al.) — builders of [Minions](approaches.md#stripe-minions); the [Part 1 blog post](https://stripe.dev/blog/minions-stripes-one-shot-end-to-end-coding-agents) is credited to Gray, a software engineer on Leverage ("surprisingly delightful internal products that Stripes can leverage"). The patterns are now the [Stripe School](approaches.md#stripe-minions).
