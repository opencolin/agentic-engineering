...
**Watch:** Blaxel, Sprites.dev, Tenki Sandbox ([Tenki Review](tenki.md) covers the bundle thesis), Cloudflare Workers / Sandboxes (edge use cases).
...
- **Tenki Runners** — *Why:* Bundled with Sandbox + Code Reviewer — the vendor-stitching pain is the explicit value prop ([Tenki Review](tenki.md)). *Caveat:* Newer entrant; the bundle is what justifies the pick, not standalone.
...
- **Tenki Code Reviewer** — *Why:* Bundle pick — pair with Tenki Sandbox + Runners for vendor-stitching savings ([Tenki Review](tenki.md)). *Caveat:* Standalone is closer to A-tier; the bundle is what pushes it up — and the bundle is new.
