...
For the model layer powering these systems (which Anthropic / Google / OpenAI / xAI models to pick, plus the open-weights options like DeepSeek, Qwen, Llama, Kimi, GLM, MiniMax, and Mistral), see [Models](/models/). For the surrounding disciplines that make any of these approaches reliable, see [Harness Engineering](/harness-engineering/), [Context Engineering](/context-engineering/), [Tool Design](/tool-design/), and [Skills](/skills/).
...
- [Cline](#cline) — 62K stars, plan-then-act with explicit user approval at every step
...
- [Letta Code](#letta-code) — 23K stars, memory-first coding agent built on the MemGPT lineage
...
- [Deep Agents (LangChain)](#deep-agents-langchain) — 23.3K stars, the LangGraph-based opinionated open harness
- [Google ADK](#google-adk-agent-development-kit) — 20K stars, Google's first-party Gemini-native framework
- [Claude Agent SDK](#claude-agent-sdk) — 7K (Python) + 1.5K (TS), Anthropic's first-party SDK — the same harness that powers Claude Code
- [Strands Agents](#strands-agents) — 5.9K stars, AWS-incubated SDK with Bedrock / Lambda / Fargate deploy templates
...
[LangChain](/who-is-who/#harrison-chase)'s cloud-native async coding agent, explicitly modeled on patterns from companies like Stripe, Ramp, and Coinbase. Uses a [multi-agent architecture](/patterns/#6-multi-agent-coordination) built on LangGraph.
...
Pioneered the "GitHub issue in, pull request out" paradigm. Published at NeurIPS 2024 by [John Yang](/who-is-who/#john-yang) and collaborators including [Shunyu Yao](/who-is-who/#shunyu-yao). Also produced mini-swe-agent, a 100-line agent scoring 74%+ on [SWE-bench](/benchmarks/#swe-bench).
...
The best open-source match for [Stripe](#stripe-minions)'s [multi-agent parallelization pattern](/patterns/#6-multi-agent-coordination). Decomposes tasks, spawns parallel agents, and autonomously handles CI fixes and merge conflicts.
...
Framework for patching code repos using LLMs. Its "patchflows" — reusable workflows combining atomic actions with LLM prompts — are the closest open-source analog to [Stripe's blueprint pattern](/patterns/#2-orchestration-models).
...
- **Roo Code** ([RooCodeInc/Roo-Code](https://github.com/RooCodeInc/Roo-Code), 24K stars) is a Cline fork with multi-mode orchestration and broader model support; teams pick one or the other, rarely both
...
- **GitHub:** https://github.com/letta-ai/letta [![stars](https://img.shields.io/github/stars/letta-ai/letta?style=social)](https://github.com/letta-ai/letta)
- **Origin:** Letta (formerly MemGPT), the lab behind the 2023 MemGPT paper

The reference implementation of *memory-first* agentic coding. Where Claude Code / Codex / Aider treat the conversation as a buffer that ends at compaction, Letta agents are persistent entities with a typed memory hierarchy (core / archival / recall / message buffer) that the agent itself reads, writes, and consolidates between sessions. The same agent that fixed a bug yesterday remembers the fix today — without `CLAUDE.md` plumbing, without a [GBrain](#gbrain)-style markdown corpus, without manual context engineering.

### Architecture

- **Letta server** — REST API; every agent is a long-lived resource you address by ID
- **Memory blocks** — labelled, agent-editable text regions (`human`, `persona`, project-specific); the agent calls `core_memory_replace` like any other tool
- **Archival storage** — Postgres + pgvector; the agent decides what to evict from core to archival
...
- Pairs naturally with [Tavily](/infrastructure/#search-apis-for-agents) for live retrieval and [Inngest](/infrastructure/#durable-execution-platforms) for durable tool calls
- Listed in [Agent Memory infrastructure](/infrastructure/#purpose-built-agent-memory) as the memory layer; this entry is the coding-agent surface
...
- **Role in agentic engineering:** Mastra is a framework for building custom agent systems, not a pre-built coding agent. Teams would use it to build their own [Minions](#stripe-minions)-like system in TypeScript, with the workflow engine serving a similar role to [Stripe's blueprints](/patterns/#2-orchestration-models).
...
- **GitHub:** https://github.com/openai/openai-agents-python [![stars](https://img.shields.io/github/stars/openai/openai-agents-python?style=social)](https://github.com/openai/openai-agents-python)
- **Origin:** OpenAI, March 2025; production successor to the educational [Swarm](https://github.com/openai/swarm) library
...
- Three of the four major model labs now ship a first-party agent framework — [Anthropic Claude Managed Agents](#claude-managed-agents), [Google ADK](#google-adk-agent-development-kit), and OpenAI Agents SDK. The convergence on the *handoff* primitive is striking: rather than rebuild LangGraph, the labs all settled on "agents that hand off to other agents, with the loop being unsurprising"
- For teams not invested in the LangChain or Microsoft Agent Framework ecosystems, this is the de-facto OpenAI-shaped path
- **Gap:** opinionated about the handoff model; teams that need a richer state-machine or graph topology will reach for [LangGraph](/infrastructure/#agent-specific-orchestration-frameworks) or [Mastra](#mastra) instead
...
- **GitHub:** https://github.com/google/adk-python [![stars](https://img.shields.io/github/stars/google/adk-python?style=social)](https://github.com/google/adk-python)
- **Origin:** Google, 2025

Google's official agent framework. Same `Agent` / `Tool` / `Session` shape as [OpenAI Agents SDK](#openai-agents-sdk) and [Strands](#strands-agents), but with deep Gemini + Vertex AI integration and first-class Agent Engine / Cloud Run deployment templates.
...
- Samples repo ([adk-samples](https://github.com/google/adk-samples), 9.4K stars) is unusually polished — production-grade reference agents for customer-service, RAG, code-review, and multi-agent decomposition
...
- **Type:** Open Source (Apache 2.0)
- **Stars:** 5.9K
- **GitHub:** https://github.com/strands-agents/sdk-python [![stars](https://img.shields.io/github/stars/strands-agents/sdk-python?style=social)](https://github.com/strands-agents/sdk-python)
- **Origin:** AWS-incubated; model-driven agent SDK released 2025

AWS's answer to [OpenAI Agents SDK](#openai-agents-sdk) and [Google ADK](#google-adk-agent-development-kit). Strands ships 13+ model providers, native MCP, multi-agent composition, and an experimental bidirectional voice-streaming mode.
...
- The reference harness in 2026. Every other harness in this section is either explicitly modeled after these primitives ([Deep Agents](#deep-agents-langchain), [Superpowers](#superpowers), [GStack](#gstack)) or reimplements the same set under different names
...
- See [Harness Engineering](/harness-engineering/) for the conceptual layer underneath this SDK — what a harness decomposes into and why Claude Code's harness wins on CORE
- **Gap:** Anthropic-locked. The SKILL.md format itself is portable (Vercel Skills installs the same format into 51+ agents), but the SDK assumes Anthropic API conventions. Provider-neutral teams reach for [Deep Agents](#deep-agents-langchain) instead
...
- **Middleware** — `before_agent`, `wrap_model_call`, `before_tools`, `after_tools` hooks compose like Express middleware. SummarizationMiddleware and FilesystemMiddleware are the headline ones
- **Sub-agents** — isolated-context children spawned via the planning loop; compressed summaries returned to the parent
...
- The right pick if you want Claude-Code-shaped ergonomics but need model neutrality, durable checkpointing, time-travel debugging, or first-class observability via [LangSmith](/infrastructure/#agent-observability--evaluation)
...
- **GitHub:** [stanfordnlp/dspy](https://github.com/stanfordnlp/dspy) [![stars](https://img.shields.io/github/stars/stanfordnlp/dspy?style=social)](https://github.com/stanfordnlp/dspy) · [gepa-ai/gepa](https://github.com/gepa-ai/gepa) [![stars](https://img.shields.io/github/stars/gepa-ai/gepa?style=social)](https://github.com/gepa-ai/gepa)
...
- **Signature** — typed input/output spec (`question -> answer: str`); the program's contract
- **Module** — `ChainOfThought`, `ReAct`, custom — composed into programs like layers
- **Optimizer** — compiles a program against your metric. `BootstrapFewShot`, `MIPROv2`, and now `GEPA` are the canonical ones
- **GEPA loop** — reflect on rollouts → mutate text parameters → keep the Pareto frontier → repeat. Optimizes whole-program text, not just a single prompt
- **DSPy 3.0** — adds streaming, async, type-safe outputs, MCP, and the `dspy.GEPA` integration
...
- See [Inspect AI](/benchmarks/#inspect-ai) for the eval-side counterpart; DSPy needs a metric, and Inspect is one place to get one with rigor
...
- **Type:** Open Source (Apache 2.0)
- **Stars:** 27.5K
- **GitHub:** [huggingface/smolagents](https://github.com/huggingface/smolagents) [![stars](https://img.shields.io/github/stars/huggingface/smolagents?style=social)](https://github.com/huggingface/smolagents)
- **Origin:** HuggingFace, December 2024

A *deliberately small* code-agent framework. The whole library is ~1,000 lines of Python — designed to be **hackable, readable, and used as a teaching tool** for how a code-execution agent actually works. The differentiator vs JSON-tool-calling frameworks: smolagents agents write their actions as Python code snippets and execute them, which is closer to how frontier code agents (Claude Code, Codex) actually behave under the hood. Multi-modal, MCP-native, and works with any LLM that supports tool calling — but production-weak compared to [Claude Agent SDK](#claude-agent-sdk) or [Deep Agents](#deep-agents-langchain).
...
- **Sandboxing** — local Python execution by default; integrations for [E2B](/sandboxes/#purpose-built-agent-sandboxes), Docker, and HuggingFace Spaces
...
- Bundled course: [HuggingFace Agents Course](https://huggingface.co/learn/agents-course) walks through smolagents, MCP, and evals end-to-end with a free certificate
...
- **Origin:** Created by [Peter Steinberger](/who-is-who/#peter-steinberger-steipete) (Austria), originally named Clawdbot (Nov 2025)
...
- **Role in agentic engineering:** OpenClaw's messaging-platform integration mirrors how Stripe engineers invoke [minions](#stripe-minions) from Slack. Its self-hosted nature and [multi-agent coordination](/patterns/#6-multi-agent-coordination) make it a potential foundation for building an agentic engineering workflow, though it requires more custom configuration than purpose-built coding agents. The skills platform and agent-to-agent tools could power a Minions-like system where coding tasks are dispatched through familiar messaging channels.
...
| **Hermes Agent** | Python | ~24.6K | Self-improving skill loop, RL fine-tuning via Atropos *(now an independent project at 165K stars — see [dedicated section](#hermes-agent))* |
...
**Hosting ecosystem:** 30+ vendors now offer OpenClaw-compatible hosting across six tiers — from turnkey platforms (ZenClaw AI, KlausAI, Coral) to self-hosted infrastructure. See the [Hosting & Execution Infrastructure](/infrastructure/#agent-hosting-execution-platforms) page for the full vendor landscape.

### The Steinberger School

> *"How would we build software in the future if tokens don't matter?"* — [Peter Steinberger](/who-is-who/#peter-steinberger-steipete)

OpenClaw's founder runs the project as a deliberate experiment in **token-unbounded agentic engineering** — what he calls his AI Software Factory. The methodology has consolidated into a recognizable *school* with its own toolchain, idioms, and operational assumptions, distinct from the [Stripe School](/schools/#the-stripe-school) (large-org devbox + blueprints), the [Tan School](/schools/#the-tan-school) (one-engineer skill packs + parallel worktrees), and the [Walking Labs / Mastery School](/schools/#the-walking-labs-mastery-school) (validator-first methodology). Reported spend is around $1.3M/month across roughly 100 Codex instances in the cloud, operated by a team of ~3. That cost is the point of the experiment, not a bug — it's a working answer to *"what's possible if inference cost stops being the binding constraint?"*
...
The fix loop is **one finding at a time, with validation** — that's a [WIP=1](/harness-engineering/#wip1-and-the-feature-list-as-primitive) discipline applied to bug remediation.
...
- **Validation-gated fixes** — patches only land if validation passes, mirroring [feature-list-as-primitive](/harness-engineering/#wip1-and-the-feature-list-as-primitive) `passing`-state gating
...
- **Browser policy** — `/browse` replaces direct [Chrome DevTools MCP](#browser-use-computer-use-frameworks) usage; the skill encodes how to use it correctly so the agent doesn't reinvent the wheel.
